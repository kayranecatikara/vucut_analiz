...
- Kalori hesaplama için sadece RGB görüntü kullanımı
"""
...
def simulate_food_detection(image_data):
    """Yemek tespiti simülasyonu - RGB görüntü ile çalışır"""
    foods_database = [
    ...
    return {
        ...
        "analysis_method": "rgb_simulation",  # RGB analiz belirteci
        "image_source": "rgb_camera"
    }

def capture_single_frame():
    """Tek bir RGB frame yakala (kalori hesaplama için - sadece RGB)"""
    global camera_mode
    
    try:
        if camera_mode == "realsense" and REALSENSE_AVAILABLE:
            return capture_realsense_rgb_frame()
        else:
            return capture_webcam_frame()
    except Exception as e:
        print(f"❌ RGB Frame yakalama hatası: {e}")
        return None

def capture_realsense_rgb_frame():
    """RealSense'den sadece RGB frame yakala (kalori hesaplama için)"""
    pipeline = None
    try:
        pipeline = rs.pipeline()
        config = rs.config()
        # Sadece RGB stream - derinlik stream yok
        ...
        if color_frame:
            ...
            print("✅ RealSense RGB fotoğraf yakalandı")
            return color_image
        
        return None
        
    except Exception as e:
        print(f"❌ RealSense RGB frame yakalama hatası: {e}")
        return None
    finally:
        if pipeline:
            try:
...
def capture_webcam_frame():
    """Webcam'den RGB frame yakala (kalori hesaplama için)"""
    cap = None
    try:
        ...
        if ret and frame is not None:
            ...
            print("✅ Webcam RGB fotoğraf yakalandı")
            return frame
        
        return None
        
    except Exception as e:
        print(f"❌ Webcam RGB frame yakalama hatası: {e}")
        return None
    finally:
        if cap:
            cap.release()
...
def process_food_photo():
    """Yemek fotoğrafını işle ve kalori hesapla - Sadece RGB görüntü kullan"""
    ...
    try:
        ...
        # Sadece RGB frame yakala
        captured_frame = capture_single_frame()
        
        if captured_frame is None:
            safe_emit('food_analysis_error', {'message': 'RGB fotoğraf çekilemedi'})
            return
        
        # RGB fotoğrafını base64'e çevir (yüksek kalite JPEG)
        _, buffer = cv2.imencode('.jpg', captured_frame, [cv2.IMWRITE_JPEG_QUALITY, 95])
        img_base64 = base64.b64encode(buffer).decode('utf-8')
        
        print("📸 RGB fotoğraf başarıyla çekildi ve encode edildi")
        ...
        # Sonuçları gönder - sadece RGB görüntü
        safe_emit('food_analysis_result', {
            'image': img_base64,  # Temiz RGB görüntü
            'analysis': food_analysis,
            'image_type': 'rgb',  # Görüntü tipini belirt
            'camera_mode': camera_mode
        })
        
        print(f"✅ RGB Kalori hesaplama tamamlandı: {food_analysis['total_calories']} kcal")
        
    except Exception as e:
        print(f"❌ RGB Yemek fotoğrafı işleme hatası: {e}")
        safe_emit('food_analysis_error', {'message': f'RGB işleme hatası: {str(e)}'})
    finally:
        calorie_calculation_active = False
...
def handle_take_food_photo(data=None):
    ...
    try:
        if not calorie_calculation_active and not test_running:
            print("📸 Kalori hesaplama için RGB fotoğraf çekiliyor...")
...
if __name__ == '__main__':
    ...
    print("   - Kalori hesaplama özelliği - SADECE RGB GÖRÜNTÜ")
    print("   - Yemek fotoğrafı çekme - SADECE RGB KAMERA")
    print()
