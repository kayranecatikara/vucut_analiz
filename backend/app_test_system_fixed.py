...
def capture_rgb_image_realsense():
    ...
    try:
        ...
        print("✅ RealSense RGB yakalama başlatıldı")
        ...
        # Son RGB frame'i yakala
        frames = pipeline.wait_for_frames(timeout_ms=5000)
        color_frame = frames.get_color_frame()
        
        if not color_frame:
            print("❌ RGB frame alınamadı")
            return None
        ...
        print("✅ RealSense RGB görüntü yakalandı")
        return color_image
        
    except Exception as e:
        print(f"❌ RealSense RGB yakalama hatası: {e}")
        return None
    finally:
        if pipeline:
            pipeline.stop()
            print("🛑 RealSense RGB yakalama durduruldu")

def capture_rgb_image_webcam():
    """Webcam'den RGB görüntü yakala"""
    cap = None
    try:
        ...
        print(f"✅ Webcam {working_camera_index} RGB yakalama başlatıldı")
        ...
        print("✅ Webcam RGB görüntü yakalandı")
        return frame
        
    except Exception as e:
        print(f"❌ Webcam RGB yakalama hatası: {e}")
        return None
    finally:
        if cap:
            cap.release()
